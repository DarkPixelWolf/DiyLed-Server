...
class AppInstance():
	...
	def sendMessage(self, message):
		global udp
		udp.sendto(message.encode('utf-8'), (self.ip, 8080))
...
class Room():
	...
	def togglePower(self, newPowerState):
		...
		for lightName in self.lights:
			...
			requests.put("http://" + lights[lightName].ip + ":80/diyledapi/" + str(hex(get_mac())) + "/updateValue", data=json.dumps(jsonData).encode('utf-8'))
...
class Scene():
	...
	def applyScene(self):
		if DEBUG:
			print("SERVER: applying Scene: " + self.name + " of " + self.room + " for " + str(len(lightStates)) + " lights")
		for light in self.lightStates:
			...
			requests.put("http://" + l.ip + ":80/diyledapi/" + str(hex(get_mac())) + "/applyScene", data=json.dumps(jsonData).encode('utf-8'))
...
class AlexaId: 
	def __init__(self, name, aId, parentLight, deleted):
		self.id = aId
		self.name = name
		self.parentLight = parentLight
...
class Config():
	...
	def getLights(self):
		...
		for lightJson in self.config["lights"]:
			cLights[lightJson["name"]] = Light(lightJson["name"], lightJson["rooms"], int(lightJson["ledCount"]), LedColor(0, 0, 0), 0, False, 0, lightJson["modes"], lightJson["ip"])
		return cLights
	...
	def getScenes(self):
		...
		for sceneJson in self.config["scenes"]:
			...
			for lsJson in sceneJson["lightStates"]:
				ls[lsJson["name"]] = {"color": LedColor(int(lsJson["color"][0]), int(lsJson["color"][1]), int(lsJson["color"][2])), "mode": str(lsJson["mode"]), "power": json.loads(str(lsJson["power"]).lower()), "brightness": int(lsJson["brightness"])}
	...
	def addScene(self, scene):
		...
		if DEBUG:
			print("CONFIG: adding Scene: (" + str(scene.name) + ") " + scene.room + " - " + str(len(scene.lightStates)) + " lights")
	...
	# -- ALEXAIDS functions
	def getAlexaIds(self):
		aIds = {}
		for aIdJson in self.config["alexaIds"]:
			aIds[aIdJson["id"]] = AlexaId(aIdJson["name"], aIdJson["id"], aIdJson["parentLight"], bool(aIdJson["deleted"]))
		return aIds
		
	def addAlexaId(self, alexaId):
		global alexa
		global lights
		alexaIdJson = {
			"id": alexaId.id,
			"name": alexaId.name,
			"parentLight": alexaId.parentLight,
			"deleted": str(alexaId.deleted)
		}
		if DEBUG:
			print("CONFIG: adding AlexaID: (" + str(alexaId.id) + ") " + alexaId.name + "[" + alexaId.parentLight + "]")
		self.config["alexaIds"].append(alexaIdJson)
		alexa.addDevice(lights[alexaIdJson["name"]].name, alexaCallback, True, iv=lights[alexaIdJson["name"]].brightness, rn=True)
		self.save()
		
	def updateAlexaId(self, alexaId):
		for i in range(len(self.config["alexaIds"])):
			if self.config["alexaIds"][i]["id"] == alexaId.id:
				self.config["alexaIds"][i]["name"] = alexaId.name
				self.config["alexaIds"][i]["parentLight"] = alexaId.parentLight
				self.config["alexaIds"][i]["deleted"] = str(alexaId.deleted)
				if i < len(alexa.devices):
					device = alexa.devices[i]
					device.name = alexaId.name
					alexa.devices[i] = device
				break
		self.save()
		
	def removeAlexaId(self, alexaId):
		global alexaIds
		if alexaId.parentLight == "":
			for aIdJson in self.config["alexaIds"]:
				if aIdJson["parentLight"] == alexaId.name:
					aIdJson["name"] = "DELETED"
					aIdJson["parentLight"] = "DELETED"
					aIdJson["deleted"] = str(True)
		alexaId.name = "DELTED"
		alexaId.parentLight = "DELETED"
		alexaId.deleted = True
		alexaIds[alexaId.id] = alexaId
		if alexaId.id < len(alexa.devices):
			device = alexa.devices[alexaId.id]
			device.deviceName = "DELETED"
			alexa.devices[alexaId.id] = device
		self.updateAlexaId(self, alexaId)
		
	def getFreeAlexaId(self):
		freeIds = []
		highestId = 0
		for aIdJson in self.config["alexaIds"]:
			if int(aIdJson["id"]) > highestId:
				highestId = int(aIdJson["id"])
			if aIdJson["deleted"] == "True":
				freeIds.append(int(aIdJson["id"]))
		if len(freeIds) == 0:
			print(len(self.config["alexaIds"]))
			if len(self.config["alexaIds"]) == 0:
				freeIds.append(highestId)
			else:
				freeIds.append(highestId+1)
		return freeIds
		
	def isInAlexaIds(self, lightName):
		for aIdJson in self.config["alexaIds"]:
			if aIdJson["name"] == lightName:
				return True
		return False
	...
	if packetType == "infoRequestPacket":
		...
		if jsonData["data"]["request"] == "room":
			...
			if not ISUDP:
				...
				handler.end_headers()
				handler.wfile.write(json.dumps(jsonReturn).encode('utf-8'))
		elif jsonData["data"]["request"] == "light":								# used as a register and setup function! lights should send an initial packet with the createPacketRequest.light id
			if not jsonData["data"]["name"] in lights:	# register if unknown
				nLight = Light(jsonData["data"]["name"], [], int(jsonData["data"]["ledCount"]), LedColor(int(jsonData["data"]["color"][0]), int(jsonData["data"]["color"][1]), int(jsonData["data"]["color"][2])), str(jsonData["data"]["mode"]), bool(jsonData["data"]["power"]), int(jsonData["data"]["brightness"]), jsonData["data"]["modes"], jsonData["data"]["ip"])
				lights[jsonData["data"]["name"]] = nLight
				print(alexaIds)
				if not config.isInAlexaIds(nLight.name):
					freeIds = config.getFreeAlexaId()
					print(freeIds)
					lowestId = min(freeIds)
					print(lowestId)
					alexaId = AlexaId(nLight.name, lowestId, "", False)
					if lowestId in alexaIds.keys():
						config.updateAlexaId(alexaId)
					else:
						config.addAlexaId(alexaId)
					alexaIds[lowestId] = alexaId
				...
				l.color = LedColor(int(jsonData["data"]["color"][0]), int(jsonData["data"]["color"][1]), int(jsonData["data"]["color"][2]))
				...
				if not config.isInAlexaIds(l.name):
					freeIds = config.getFreeAlexaId()
					lowestId = min(freeIds)
					alexaId = AlexaId(l.name, lowestId, "", False)
					if lowestId in alexaIds.keys():
						config.updateAlexaId(alexaId)
					else:
						config.addAlexaId(alexaId)
		...
		if jsonData["data"]["request"] == "lightStatesOfScene":
			...
			ls = {}
			for lsJson in jsonData["data"]["lightStates"]:
				ls[lsJson["name"], {"color": LedColor(int(lsJson["color"][0]), int(lsJson["color"][1]), int(lsJson["color"][2])), "mode": int(lsJson["mode"]), "power": json.loads(str(lsJson["power"]).lower()), "brightness": int(lsJson["brightness"])}]
		...
		if jsonData["data"]["request"] == "light":
			...
			if config.isInAlexaIds(light.name):
				for i in alexaIds.keys():
					if alexaIds[i].name == light.name:
						config.removeAlexaId(alexaIds[i])
						break
			jsonReturn = {
				"id": "successPacket",
				"data": {
		...
		if jsonData["data"]["request"] == "room":
			...
			if jsonData["data"]["key"] == "power":
				...
				if config.config["server"]["mqttauth"] == "True":
					client.publish(jsonData["data"]["name"], payload=str(lights[jsonData["data"]["name"]].power).lower(), qos=0, retain=False)
				for room in lights[jsonData["data"]["name"]].rooms:
					rooms[room].updatePowerState()
				response = requests.put("http://" + lights[jsonData["data"]["name"]].ip + ":80/diyledapi/" + str(hex(get_mac())) + "/updateValue", data=json.dumps(jsonData).encode('utf-8'))
			...
			if jsonData["data"]["key"] == "color":
				lights[jsonData["data"]["name"]].color = LedColor(int(jsonData["data"]["value"][0]), int(jsonData["data"]["value"][1]), int(jsonData["data"]["value"][2]))
				response = requests.put("http://" + lights[jsonData["data"]["name"]].ip + ":80/diyledapi/" + str(hex(get_mac())) + "/updateValue", data=json.dumps(jsonData).encode('utf-8'))
				response = json.loads(response.content.decode('utf-8'))
				jsonReturn = ""
				if response["id"] == "successPacket":				
...
class httpHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self):
		...
		if (path == "/espalexa"):
			alexa.servePage(self)
			return
		elif (path == "/description.xml"):
			alexa.serveDescription(self)
			return
		elif (path.startswith("/api")):
			content_len = int(self.headers.get('Content-Length', 0))
			post_body = self.rfile.read(content_len)
			alexa.handleAlexaApiCall(path, post_body.decode('utf-8'), self)
			return
			...
			for lightName in lights:
				response = response + str(lightName) + " - Power: " + str(lights[lightName].power).lower() + " | Brightness: " + str(lights[lightName].brightness) + " | Mode: " + str(lights[lightName].mode) + " | Color: " + str(lights[lightName].color.r) + ", " + str(lights[lightName].color.g) + ", " + str(lights[lightName].color.b) + "\r\n"
			...
			if (json.loads(post_body)):
				...
				for app in appInstances:
					if appInstances[app].ip != ip:
						...
						app.sendMessage(json.dumps(jsonData))
			return
		if DEBUG:
			print("HTTP: error handling request from " + str(self.client_address))
		self.send_response(200)
	...
	while True:
		...
		if (request):
			...
			if (request.find("HTTP/1.1 200 OK") >= 0) and (request.find("urn:diyleddevice:light") >= 0):
				...
				udp.sendto(response.encode('utf-8'), (ip, MCAST_PORT))
			if (request.find("M-SEARCH * HTTP/1.1") >= 0) and ((request.find("upnp:rootdevice") > 0) or (request.find("asic:1") > 0)):
				if DEBUG:
					print("UDP: responding ESPALEXA CALLL of " + str(request_addr))
				alexa.respondToSearch(request_addr, udp)
...
def alexaCallback(brightness, rgb, name):
	global lights
	if name in lights.keys():
		light = lights[name]
		jsonData = []
		if brightness == 0:
			light.power = False
		...
		lights[name] = light
		for j in jsonData:
			response = requests.put("http://" + light.ip + ":80/diyledapi/" + str(hex(get_mac())) + "/updateValue", data=json.dumps(j).encode('utf-8'))
			response = response.content.decode('utf-8')
		return response.find("successPacket") >= 0
	else:
		return False
...
alexaIds = config.getAlexaIds()

newLights = []
searching = False

alexa = Espalexa(MAXDEVICES = 1000, DEBUG = False, CUSTOMHTTP = True, CUSTOMUDP = True)
for x in range(len(alexaIds.keys())):
	if not alexaIds[x].name == "DELETED":
		alexa.addDevice(lights[alexaIds[x].name].name, alexaCallback, True, iv=lights[alexaIds[x].name].brightness, rn=True)
	else:
		alexa.addDevice("DELETED", alexaCallback, True, iv=0, rn=True)
alexa.begin()
...
if DEBUG:
	...
	print("  -> " + str(len(alexaIds)) + " alexaIds")
